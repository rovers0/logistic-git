...
const actions = {
    ...
    getDriverAll({ commit }, data) {
        commit('setLoading', true, { root: true });
        return axiosClient.get("driver/all", {params: data}).then((res) => {
            commit('setLoading', false, { root: true });
            return res;
        });
    },
    ...
    deleteDriverMedia({ commit }, {id, mediaId}) {
        commit('setLoading', true, { root: true });
        return axiosClient.delete("driver/" + id + "/destroy-media/" + mediaId,).then((res) => {
            commit('setLoading', false, { root: true });
            return res;
        });
    },
